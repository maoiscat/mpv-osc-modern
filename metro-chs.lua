...
local user_opts = {
    ...
    logotext = "{\\fs30}Drop files or URLs to play here."
								-- this text appears when mpv starts
}

-- Internal control opts
local ctrl_opts = {
	v_paused	= "always",	-- visibility on paused
	v_play		= user_opts.visibility,	--visibility back to play
            ...
            if not (element.slider.markerF == nil) and (slider_lo.gap > 0) then
                ...
                for _,marker in pairs(markers) do
                    if (marker > element.slider.min.value) and
                        ...
                        if (slider_lo.gap > 4) then -- draw triangles

                            --top
                            if (slider_lo.nibbles_top) then
                                static_ass:move_to(s - 3, foV - 5)
                                static_ass:line_to(s + 3, foV - 5)
                                static_ass:line_to(s, foV - 1)
                            end

                            --bottom
                            if (slider_lo.nibbles_bottom) then
                                static_ass:move_to(s - 3, elem_geo.h - foV + 5)
								static_ass:line_to(s, elem_geo.h - foV + 1)
                                static_ass:line_to(s + 3, elem_geo.h - foV + 5)
                            end
        ...
        if (element.type == "slider") then
            ...
            if pos then
				...
				ass_draw_rr_h_cw(elem_ass, xp - rh, foH - rh,
								 xp + rh, foH + rh,
								 rh, false)
				elem_ass:rect_cw(s_min, foV, xp, elem_geo.h-foV)
            end
...
function window_controls()
    ...
    lo = add_layout("close")
    lo.geometry = {x = controlbox_left + 115, y = button_y, an = 5, w = 40, h = wc_geo.h}
    ...
    lo.geometry = {x = controlbox_left + 27, y = button_y, an = 5, w = 40, h = wc_geo.h}
    lo.style = osc_styles.WinCtrl
    lo.alpha[3] = 0
    ...
    lo.geometry = {x = controlbox_left + 69, y = button_y, an = 5, w = 40, h = wc_geo.h}
    lo.style = osc_styles.WinCtrl
    lo.alpha[3] = 0
end
...
layouts = function ()
    ...
    -- area for show/hide
    add_area("showhide", 0, 0, osc_geo.w, osc_geo.h)
...
function osc_init()
    ...
    ne.content = function ()
        local title = mp.command_native({"expand-text", user_opts.title .. " " .. osc_param.playresx .. " " .. osc_param.playresy})
...
function pause_state(name, enabled)
    ...
    if enabled then
		visibility_mode(ctrl_opts.v_paused, true)
		show_osc()
	else
		visibility_mode(ctrl_opts.v_play, true)
	end
    request_tick()
end
